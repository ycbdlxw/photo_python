...
cnx = mysql.connector.connect(user='root', password='Ycbd74mysql!@#',
 ...
 # Get a cursor object
cursor = cnx.cursor()
 # Create the albumtree table if it doesn't exist
create_table_query = """
CREATE TABLE IF NOT EXISTS albumtree (
...
query = 'SELECT id, name FROM albumtree'
...
query = 'SELECT albums FROM img_info_data'
cursor.execute(query)
rows = cursor.fetchall()
